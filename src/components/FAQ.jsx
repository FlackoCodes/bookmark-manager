...
const FAQ = () => {
  ...
  const toggleAccordion = (index) => {
    setToggledItems(
      (
        prevToggledItems // preveToggledItems is the previous state
          ...
          {accordion.map((data, i) => (
            <div key={data.id}>
              ...
              <div
                ...
                className="question"
              >
                <h6 className="quest">{data.question}</h6>
                {toggledItems[i] ? (
                  <FaChevronDown color="blue" />
                ) : (
                  <FaChevronUp color="hsl(0, 94%, 66%)" />
                )}
              </div>
              <p
                className={toggledItems[i] ? "answer" : "answer reveal-answer"}
              >
                {data.answer}
              </p>
            </div>
